const { SimpleLinkedList, Element } = require('./ll.js');
...
describe("Simple Linked List", () => {
  ...
  test("element tail", () => {
    let element = new Element(1);
    expect(element.isTail()).toBe(true); // tail returns true if it is the last element added
  });
  ...
  }); //{1,next => null} {2,next => {1,next => null} } element2 =>  element1 => null next returns the the next element
  ...
  test("to a: of a 2 element list", () => {
    ...
    expect(list).toEqual([1, 2]);//{2, 1} => [2, 1]
  });

  test("reverse 2 element list", () => {
    let list = SimpleLinkedList.fromArray([1, 2]); ///{1, 2}
